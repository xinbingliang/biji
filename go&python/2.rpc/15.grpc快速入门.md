...
func main() {
	conn, err := grpc.Dial("127.0.0.1:5001", grpc.WithInsecure()) //拨号
	if err != nil {
		panic(err)
	}
	defer conn.Close()
	...
	//r, err := c.GetStream(context.Background(), &proto.StreamReqData{Data: "xin"})
	//if err != nil{
	//	panic(err)
	//}
	//for {
	...
	go func() {
		defer wg.Done()
		for {
			_ = allstr.Send(&proto.StreamReqData{Data: "客户端消息"})
			time.Sleep(time.Second)
		}
	}()
}
